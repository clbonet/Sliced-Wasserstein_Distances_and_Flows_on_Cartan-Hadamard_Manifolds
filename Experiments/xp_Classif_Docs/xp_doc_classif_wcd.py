...
#         w = mat_contents["BOW_X"][0]
    ...
    for k in range(n_try):        
        for i in range(len(idx_train)):
            ...
            A = torch.from_numpy(np.loadtxt("./results"+str_dataset+"/A_wcd"+str_d+str_dataset+"_i"+str(i))).to(device)
            
            if args.loss == "w2":
                dist_mat = compute_matrix_w(X, w, A, torch.ones(n_words, device=device), idx_docs, device=device)
            elif args.loss == "sw":
                dist_mat = compute_matrix_sw(X, w, A@A.T + 1e-10*torch.eye(300, device=device), 
                                             torch.ones(n_words, device=device), idx_docs, n_projs, device=device)

            np.savetxt("./results"+str_dataset+"/d_swmd_wcd"+str_d+str_loss+str_dataset+str_projs+"_i"+str(i)+"_k"+str(k), dist_mat)
