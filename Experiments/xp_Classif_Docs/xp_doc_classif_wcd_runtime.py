...
#         w = mat_contents["BOW_X"][0]
    ...
    for k in range(n_try):        
        for i in range(1):            
            ...
            if args.loss == "w2":
                dist_mat, ts = compute_matrix_w(X, w, A, torch.ones(n_words, device=device), idx_docs, device=device, get_time=True)
            elif args.loss == "sw":
                dist_mat, ts = compute_matrix_sw(X, w, A@A.T, torch.ones(n_words, device=device), idx_docs, n_projs, device=device, get_time=True)


            np.savetxt("./results_time"+"/ts_swmd_wcd"+str_d+str_loss+str_dataset+str_projs+"_i"+str(i)+"_k"+str(k), ts)
